import { calculateWinner } from './resultCalc';
...
export function calculateNextMove(square , winSymbol){
	var state = square.clone2DArray(),
		bestMove;
	bestMove = minMax(state , winSymbol , winSymbol==="X" ? "O" : "X" , winSymbol, 15);
...
function minMax(state , playerSymbol, opponentSymbol , turnSymbol , score  ){
	...
	allPossibleEmptySquare.forEach(function(possibleMove){
		...
		minMaxResult = minMax(newState , playerSymbol , opponentSymbol , turnSymbol ==="X" ? "O" : "X" , score-1);
		minMaxResultObj = {
	...
	if(turnSymbol===playerSymbol){
		let maxScoreMove = getMoveWithMaxScore(minMaxResultArray);
		return {
			score : maxScoreMove.score,
			move : maxScoreMove.move
		};
	}else if(turnSymbol===opponentSymbol){
		let minScoreMove = getMoveWithMinScore(minMaxResultArray);
		return {
